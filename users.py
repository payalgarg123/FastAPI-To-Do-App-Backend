...
async def create_user(db: db_dependency, create_user_request: CreateUserRequest):
    ...
    if db.query(Users).filter(Users.username == create_user_request.username).first() or db.query(Users).filter(Users.email == create_user_request.email).first():
...
async def update_user_info(user: user_dependency, db: db_dependency, user_info_update: UserInfoUpdate):
    ...
    if any(getattr(user_info_update, field) is not None and getattr(user_info_update, field) != getattr(user_model,
        ...
        if db.query(Users).filter(Users.username == user_info_update.username).first() or db.query(Users).filter(Users.email == user_info_update.email).first():
